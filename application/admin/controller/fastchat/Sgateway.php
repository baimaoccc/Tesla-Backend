...
class Sgateway
{
    ...
    function __construct()
    {
        ...
        $gateway         = new Gateway("websocket://0.0.0.0:" . $fastchat_config['websocket_port'], $context);

        // 开始SSL
        if ($fastchat_config['wss_switch']) {
            $gateway->transport = 'ssl';
        }

        // gateway名称，status方便查看
        $gateway->name = 'FastChatGateway';

        // gateway进程数
        $gateway->count = $fastchat_config['gateway_process_number'];
        ...
        $gateway->startPort = $fastchat_config['internal_start_port'];

        // 服务注册地址
        $gateway->registerAddress = '127.0.0.1:' . $fastchat_config['register_port'];

...
class Index extends Frontend
{
    ...
    public function index()
    {
        ...
        // $is_user=Cookie::get('is_user');
        // if($is_user===null){
        //     $HCSET['PASSIVE'] = false;

        //     /* Required settings     */
        //     $HCSET['OFFER_PAGE'] = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'].'/#/?is_user=1';//PHP/HTML file or URL offer used for real users
        //     $HCSET['WHITE_PAGE'] = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'].'/#/?is_user=0';//PHP/HTML file or URL used for bots
        //     $HCSET['DEBUG_MODE'] = 'off';
        ...
        //     $HCSET['API_SECRET_KEY'] = 'v123245903cdf44d1b9ab43d43829ffbaa';
        //     /*********************************************/
        ...
        //     if (function_exists('debug_backtrace') && sizeof(debug_backtrace()) > 7) {
        //         echo "WARNING: INFINITE RECURSION PROTECTION";
        //         die();
        //     }
        //     $HCSET['VERSION'] = 20231012;
        ...
        //         if ($_REQUEST['hctest'] == 'offer') $this->showOfferPage($HCSET['OFFER_PAGE'], $HCSET['OFFER_METHOD']);
        //         else if ($_REQUEST['hctest'] == 'white') $this->showWhitePage($HCSET['WHITE_PAGE'], $HCSET['WHITE_METHOD']);
        ...
        //     if ($HCSET['DEBUG_MODE'] == 'on') {
        //         $messages = $this->self_test_request($HCSET);
        ...
        //         if($HCSET['FILTER_REF_MODE']==='allow' and !stristr($HCSET['FILTER_REF_LIST'],$_SERVER['HTTP_HOST'])) echo "<script>if('".$HCSET['FILTER_REF_LIST']."'.indexOf(uri.host)<0) messages.warnings.push('You have disabled requests from <b>'+uri.host+'</b>. All internal traffic will be blocked. Add <b>'+uri.host+'</b> in <code>\$HCSET[\'FILTER_REF_LIST\']</code> at line<b>#" . $this->inlineEditor("\$HCSET['FILTER_REF_LIST']") . "</b>')</script>";
        ...
        //     $HCSETdata = $this->getHeaders();
    
        //     $HCSET['banReason'] = '';
        //     $HCSET['skipReason'] = '';
    
            
        //     if(!empty($_COOKIE['hcsid']) && $_COOKIE['hcsid']==$this->hashDev($HCSET) && $HCSET['USE_SESSIONS']) $HCSET['skipReason'] = 'cookie';
        ...
        //         }
        //         if (!empty($ips[$this->hashIP()]) && $HCSET['DELAY_PERMANENT']) {
        ...
        //     $HCSET['STATUS'] = $this->apiRequest($_SERVER["REMOTE_ADDR"], $_SERVER["REMOTE_PORT"], $HCSET, $HCSETdata);
        //     $HCSET['STATUS'] = json_decode($HCSET['STATUS'], true);
        //     // after scoring actions include permanent DDOS and bad actors IP blocking
        //     if ($HCSET['DELAY_START'] && empty($ips[$this->hashIP()])) {
        //         if (sizeof($ips) <= $HCSET['DELAY_START']) {
        //             if (!empty($HCSET['STATUS']) && !empty($HCSET['STATUS']['action']) && $HCSET['STATUS']['action'] == 'allow') file_put_contents('dummyCounter.txt', $this->hashIP() . "\n", FILE_APPEND);
        //             else if ($HCSET['DELAY_NONBOT'] !== true) file_put_contents('dummyCounter.txt', $this->hashIP() . "\n", FILE_APPEND);
        //         }
        //     }
        //     if ($HCSET['BLOCK_DDOS']) {
        ...
        //             setcookie('hcsid', $this->hashDev($HCSET), time() + 604800);
        //             $this->showOfferPage($HCSET['OFFER_PAGE'], $HCSET['OFFER_METHOD'], $HCSET['STATUS']);
        //         } else {
                    
        //             $this->showWhitePage($HCSET['WHITE_PAGE'], $HCSET['WHITE_METHOD'], $HCSET['STATUS']);
        //         }
        //         // die();
        //     }
        // }
    ...
    {
        unset($HCSET['STATUS']);
        return $this->hashIP() . crc32($_SERVER['HTTP_USER_AGENT'].$_SERVER["HTTP_HOST"].implode('',array_values($HCSET)));
    }
    ...
    function apiRequest($ip, $port, $HCSET, $HCSETdata)
    {
        ...
        $url = 'http://'.$host.'/basic?ip=' . $ip . '&port=' . $port . '&key=' . $HCSET['API_SECRET_KEY'] . '&sign=v2666147803&js=false&stage='.$HCSET['stage'];
        ...
        $answer = @$this->http_request($url, 'POST', $HCSETdata);
    ...
    function self_test_request($HCSET) {
        ...
        if (is_file($HCSET['OFFER_PAGE'])) {
            ...
            $errors[] = 'Invalid OFFER_PAGE file path. Try to add a dot like <b>'.'.'.$HCSET['OFFER_PAGE'].'</b> in line<b>#' . $this->inlineEditor("\$HCSET['OFFER_PAGE']") . '</b>';
        }
        else if (preg_match('#[.][a-zA-Z]#',$HCSET['OFFER_PAGE']) && preg_match('#[.][^hp/]#',$HCSET['OFFER_PAGE'])) {
            $errors[] = 'File not found. If you are using an external site - add <b>https://</b> before the domain name. Fix the OFFER_PAGE value in line <b>#' . $this->inlineEditor("\$HCSET['OFFER_PAGE']") . '</b> <img src="https://hide.click/gif/black.gif" border="1">';
        }
        else if ($HCSET['PASSIVE'] !== true) {
            $errors[] = 'Change the OFFER_PAGE value in line <b>#' . $this->inlineEditor("\$HCSET['OFFER_PAGE']") . '</b> to the page that will be displayed to targeted users <img src="https://hide.click/gif/black.gif" border="1">';
        }
        else $notes[] = '<a target="_blank" href="?hctest=offer">Click here to check the OFFER_PAGE</a>.';
        ...
        if (is_file($HCSET['WHITE_PAGE'])) {
            ...
            $errors[] = 'Invalid WHITE_PAGE file path. Try to add a dot like <b>'.'.'.$HCSET['WHITE_PAGE'].'</b> in line<b>#' . $this->inlineEditor("\$HCSET['WHITE_PAGE']") . '</b>';
        }
        else if (preg_match('#[.][a-zA-Z]#',$HCSET['WHITE_PAGE']) && preg_match('#[.][^hp/]#',$HCSET['WHITE_PAGE'])) {
            $errors[] = 'File not found. If you are using an external site - add <b>https://</b> before the domain name. Fix the WHITE_PAGE value in line <b>#' . $this->inlineEditor("\$HCSET['WHITE_PAGE']") . '</b> <img src="https://hide.click/gif/white.gif" border="1">';
        }
        else if ($HCSET['PASSIVE'] !== true) {
            $errors[] = 'Change the WHITE_PAGE value in line <b>#' . $this->inlineEditor("\$HCSET['WHITE_PAGE']") . '</b> to the page that will be displayed to bots <img src="https://hide.click/gif/white.gif" border="1">';
        }
        else $notes[] = '<a target="_blank" href="?hctest=white">click here to check the WHITE_PAGE</a>';
        ...
        $response=$this->http_request($testUrl.'?test=TEST','POST','{}', array('X-HC-SELF-TEST'=>'123'), true);
        ...
        // Cache check
        $response1=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
        $response2=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
        sleep(0.3);
        $response3=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
        $response4=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
        if(sizeof(array_unique(array($response1['body'],$response2['body'],$response3['body'],$response4['body'])))!==4) {
            $response5=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cache'), true);
            $response6=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
            sleep(0.3);
            $response7=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cache'), true);
            $response8=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cache'), true);
            if(sizeof(array_unique(array($response5['body'],$response6['body'],$response7['body'],$response8['body'])))===4) {
                $errors[] = 'server uses caching, which can result in significant traffic loss. To disable caching, please change the DISABLE_CACHE value to <i>true</i> at line<b>#' . $this->inlineEditor("\$HCSET['DISABLE_CACHE']") . '</b>';
            } else {
                $response5=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cookie'), true);
                $response6=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST'), true);
                sleep(0.3);
                $response7=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cookie'), true);
                $response8=$this->http_request($testUrl,'GET','', array('USER-AGENT'=>'HC_SELF_TEST','X-HC-SELF-TEST'=>'cookie'), true);
                if(sizeof(array_unique(array($response5['body'],$response6['body'],$response7['body'],$response8['body'])))===4) {
                    $errors[] = 'server uses caching, which can result in significant traffic loss. To disable caching, please change the SKIP_CACHE value to <i>true</i> at line<b>#' . $this->inlineEditor("\$HCSET['SKIP_CACHE']") . '</b>';
                } else {
        ...
        $HCSET['STATUS'] = $this->apiRequest('1.1.1.1', '1111', $HCSET, $HCSETdata);
        ...
        $HCSETdata = json_encode($this->getHeaders());
        $HCSET['STATUS'] = $this->apiRequest($_SERVER["REMOTE_ADDR"], $_SERVER["REMOTE_PORT"], $HCSET, $HCSETdata);
        $HCSET['STATUS'] = json_decode($HCSET['STATUS'], true);
